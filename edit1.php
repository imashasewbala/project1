...
if (isset($_POST["submit"])) {
   ...
   $sql = "UPDATE `crud2` SET `first_name`='$first_name',`email`='$email',`message`='$message',`contact_number`='$contact_number' WHERE id = $id";

    $result = mysqli_query($conn, $sql);

    if ($result) {
        header("Location: table1.php?msg=Record updated successfully");
...
$sql = "SELECT * FROM `crud2` WHERE id = $id LIMIT 1";
$result = mysqli_query($conn, $sql);
 ...
 <div class="col">
    ...
    <input type="text" class="form-control" name="email" value="<?php echo isset($row['email']) ? htmlspecialchars($row['email']) : ''; ?>">
             ...
             <div class="col">
                  ...
                  <input type="text" class="form-control" name="message" value="<?php echo isset($row['message']) ? htmlspecialchars($row['message']) : ''; ?>">
               </div>
